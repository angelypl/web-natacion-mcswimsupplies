...
export default async function HorariosPage({
  ...
  searchParams: Promise<{ branchId?: string; status?: string }>;
}) {
  const { branchId, status } = await searchParams;
  const branches = await db.orm.public.Branch.orderBy((b) => b.name.asc()).all();

  let classesQuery = db.orm.public.SwimClass.include("branch").include("enrollments", (e) =>
    e.where({ isActive: true }).count()
  );
  if (branchId) {
    classesQuery = classesQuery.where({ branchId: Number(branchId) });
  }
  if (status === "active") {
    classesQuery = classesQuery.where({ isActive: true });
  } else if (status === "inactive") {
    classesQuery = classesQuery.where({ isActive: false });
  }
  ...
  return (
    <div>
      ...
      <form className="mb-4 flex flex-wrap items-center gap-2" method="GET">
        ...
        <label htmlFor="status" className="text-sm text-slate-600 ml-2">
          Estado:
        </label>
        <select
          id="status"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {classes.map((c) => {
              const isFull = c.enrollments >= c.capacity;
              return (
                <tr key={c.id} className={c.isActive ? "" : "opacity-50"}>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <Link href={`/admin/horarios/${c.id}`} className="text-cyan-700 hover:underline mr-3">
                      Roster
                    </Link>
                    <Link
